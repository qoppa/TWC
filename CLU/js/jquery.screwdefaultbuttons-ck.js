...
(function($) {
	...
	options = $.extend($.fn.screwDefaultButtons.defaults, options);
		...
		if($(this).is(":radio")){
			...
			$('.styledRadio')parsys section twc-box-column({backgroundImage:uncheckedImage, width: width, height:height});
			$(radioButton).filter(':checked').parent().addClass('checked')parsys section twc-box-column({"background-image":checkedImage});
			
			
			if (disabledImage !== false || disabledCheckImage !== false ){
				$(radioButton).filter(':disabled').each(function(){
					if ($(this).is(':checked')){
						$(this).parent().addClass('disabled')parsys section twc-box-column({"background-image":disabledCheckImage});	
					}
					else {
						$(this).parent().addClass('disabled')parsys section twc-box-column({"background-image":disabledImage});	
					}
				});
			}
			...
			$('.styledRadio').click(function(){
				...
				if(!($(this).hasClass('disabled'))){
					...
					if(!($(this).hasClass('checked'))){ 
						...
						parsys section twc-box-column({backgroundImage:checkedImage})
						.find('input:radio')
							...
							if(otherCheckName == thisCheckName){
								if(!($(this).hasClass('selected'))){
									if($(this).hasClass('disabled')){
										...
										parsys section twc-box-column({backgroundImage:disabledImage});
									}
									else {
										$(this).removeClass('checked')
										parsys section twc-box-column({backgroundImage:uncheckedImage});
									}	
								}
							}
						});
			...
			$('.styledCheckbox')parsys section twc-box-column({backgroundImage:uncheckedImage, width: width, height:height});
			$(checkbox).filter(':checked').parent().addClass('checked')parsys section twc-box-column({"background-image":checkedImage});
			...
			$('.styledCheckbox').click(function(){
				...
				if(!($(this).hasClass('disabled'))){
					...
					if(!($(this).hasClass('checked'))){
						...
						parsys section twc-box-column({"background-image":checkedImage})
						.find('input:checkbox')
							.attr('checked','checked')
							.trigger('change');	
					}
						...
						$(this).removeClass('checked')
						parsys section twc-box-column({"background-image":uncheckedImage})
						.find('input:checkbox')
		...
		$('.styledRadio')parsys section twc-box-column({'cursor':'pointer', "background-repeat":"no-repeat"});
		$('.styledCheckbox')parsys section twc-box-column({'cursor':'pointer', "background-repeat":"no-repeat"});
