...
/*
 * TWC Overlay with COOKIE_NAME fix
 *
 * Copyright 1997-2009 Day Management AG
 * Barfuesserplatz 6, 4001 Basel, Switzerland
 * All Rights Reserved.
...
CQ_Analytics.SessionPersistence = function() {
    return {
        COOKIE_NAME: (CQURLInfo.runModes.match(/author/) ? "SessionPersistence-author" : "SessionPersistence-publish"),
...
function initializeTeaserLoader(allTeasers, strategyName, targetElementId, isEditMode) {
    isEditMode = CQ.Ext && (isEditMode == "true" || isEditMode === true);

    if( window.CQ_Analytics ) {
        var toExecute = function() {
        	//disabling all modes for returned teasers.
        	
		//var TEASER_SUFFIX = "/_jcr_content.content.html?wcmmode=disabled";
        	//Older iterations of this:
		var TEASER_SUFFIX = "/_jcr_content.content.html"+ (isEditMode ? "?wcmmode=disabled" : "");
        	//OTB:
           // var TEASER_SUFFIX = "/_jcr_content.content.html";
            ...
            var loadTeasers = function() {
                ...
                if (teasers.length > 0) {
                    ...
                    if (!currentVisibleTeaser || currentVisibleTeaser.path != teaserToShow.path) {
                        ...
                        if(window.CQ_trackTeasersStats) {
        ...
        if (options.event) { 
            ...
            if (options.values) {
                for (var j in options.values) {
                    dataMgr.setProperty(j,options. values[j]);
                }
            }
